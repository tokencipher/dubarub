...
<script>
  ...
  */
      ...
      if (this.readyState == 4 && this.status == 200) {
        ...
        for ( var x = 0; x < postCnt; x++ ) {
    	    ...
    	    if (obj[x].image == "true") {
              ...
              '<span style="float:right;color:blue;text-decoration:underline;"><a onclick="toggleCommentBox(event,' + obj[x].p_id + ')" id="addComment" href="user.php?action=Add%20Comment&pid=' + obj[x].p_id + '">Add Comment</a></span>' +
          ...
          for ( var i = 0; i < len; i++) {
            ...
            if ( comments[i].u_id == user_id ) {
              $('#post' + comments[i].p_id).find('.post_comments').append('<div id="comment' + comments[i].c_id +  '" class="media">' +
 			  ...
 			  '<div onclick="removeComment(this)" data-commid="' + comments[i].c_id + '" id="remove_comment' + comments[i].c_id + '" style="position:relative;bottom:22px;margin-right:10px;float:right">' + 
  ...
  function removeComment(element) {
    ...
    var remove_flag = Boolean("<?php (isset($_SESSION['user_id']) ? true : false); ?>");
